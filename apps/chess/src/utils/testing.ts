...
const t = fs
  .readdirSync(relative(process.cwd(), "public/pieces"))
  .map(
    p =>
      [
        `${p.split(".")?.[0]?.replace("-", "_")}`,
        `https://raw.githubusercontent.com/DopamineDriven/portfolio-2025/master/apps/chess/public/pieces/${p.split(".")?.[0]}.svg`
      ] as const
  );
