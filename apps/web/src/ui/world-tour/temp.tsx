...
const WorldTour: FC = () => {
  ...
  const [height, setHeight] = useState(337.5); // the value of taking width=600*(9/16);
  const createVisualization = useCallback(
    (worldData: World110m) => {
      setIsTourRunning(true);
      setWidth(600);
      setHeight(window.innerWidth >= 640 ? width * (9 / 16) : width);
      ...
      ) as FeatureCollection<Geometry, JSONDATA>;
      ...
      const rotateAndZoomToCountry = async (
        ...
        const [currentLon, currentLat, currentGamma] = projection.rotate();
          ...
          transition()
            ...
            .tween("rotate", () => {
              return (t: number) => {
                ...
                projection.rotate([-lonI, -latI, currentGamma]).scale(scale);
    ...
    [isoHelper, earthDefault, MAX_SCALE, MIN_SCALE, TINY_COUNTRY_SCALE, height, width]
  );
  ...
  useEffect(() => {
    if (!isTourRunning && containerRef.current) {
      ...
      const rotationTimer = timer(elapsed => {
        const rotation = [(elapsed * 0.01) % 360, -20, 0] as [
          number,
          number,
          number
        ];
        ...
        <div className="mx-auto w-full justify-center">
          ...
          <BreakoutWrapper>
            <div className="mx-auto max-w-5xl">
              <motion.div
                ref={containerRef}
                className="mx-auto aspect-square w-full sm:aspect-video py-5"
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ duration: 0.8 }}>
                <svg ref={d3ContainerRef} className="h-full w-full py-10" />
              </motion.div>
            </div>
          </BreakoutWrapper>
        </div>
