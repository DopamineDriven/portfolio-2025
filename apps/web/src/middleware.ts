...
// export function middleware(request: NextRequest) {
//   // 1) If it's a bot/crawler, do not rewrite (avoid SEO issues).
//   const hasCookieVal = request.cookies.get("has-visited");

//   const pathname = request.nextUrl.pathname;
  
//   if (!hasCookieVal) {
//     const clone = request.nextUrl.clone();
//     clone.pathname = "/elevator";
//     const res = NextResponse.rewrite(clone);
//     res.cookies.set("poi", decodeURI(pathname));
//     console.log("[SERVER] !hasCookieVal first if block check:", {
//       poi: res.cookies.get("poi")?.value ?? "no poi"
//     });
//     return res;
//   }
